...
export function ImagePlaceholder({ note }: { note: string }) {
  return (
    <div className="sig-placeholder" role="note">
      <span className="sig-placeholder__tag">Image to add</span>
      <p className="sig-placeholder__note">{note}</p>
    </div>
  );
}

/** Markdown with image resolution, a missing-image guard and note detection. */
export function SigMarkdown({
  text,
    ...
    <div className="sig-blocks">
      {blocks.map((block, i) => {
        switch (block.type) {
          case 'intro':
            return (
              <section className="sig-block" key={i}>
                {block.eyebrow && <p className="sig-eyebrow">{block.eyebrow}</p>}
                <h2 className="sig-block__heading">{block.heading}</h2>
                {block.text && <p className="sig-block__text">{block.text}</p>}
              </section>
            );
              ...
              <section className="sig-block" key={i}>
                <div className="sig-stats">
                  {block.cards.map((card, j) => {
                    ...
                    return (
                      <div className="sig-stat" key={j}>
                        {Icon && <Icon size={20} aria-hidden="true" />}
                        <strong>{card.title}</strong>
                        {card.text && <span>{card.text}</span>}
                        {card.note && <em className="sig-stat__note">{card.note}</em>}
                      </div>
                    );
                  })}
                </div>
            ...
            return (
              <section className="sig-block" key={i}>
                {(block.eyebrow || block.heading || block.text) && (
                  <div className="sig-block__head">
                    {block.eyebrow && <p className="sig-eyebrow">{block.eyebrow}</p>}
                    {block.heading && <h2 className="sig-block__heading">{block.heading}</h2>}
                    {block.text && <p className="sig-block__text">{block.text}</p>}
                  </div>
                )}
                <div className="sig-gallery">
                  {block.items.map((item, j) => {
                    ...
                    return (
                      <article className="sig-gallery__item" key={j}>
                        {src ? (
                        ...
                        {(item.title || item.description) && (
                          <div className="sig-gallery__body">
              ...
              <section className="sig-block" key={i}>
                <blockquote className="sig-quote">
                  ...
                  {(block.attribution || block.role) && (
                    <footer className="sig-quote__by">
                      {block.attribution}
                ...
                {block.image && (
                  resolveImage(block.image)
                    ? <img className="sig-quote__img" src={resolveImage(block.image)} alt={block.attribution || ''} loading="lazy" />
            ...
            return (
              <section className="sig-block" key={i}>
                ...
                <div className="sig-metrics">
                  {block.items.map((m, j) => (
                    <div className="sig-metric" key={j}>
              ...
              <section className="sig-block" key={i}>
                ...
                <ol className="sig-steps">
                  {block.items.map((s, j) => {
                    ...
                    return (
                      <li className="sig-step" key={j}>
                        <span className="sig-step__num">{String(j + 1).padStart(2, '0')}</span>
              ...
              <section className="sig-block" key={i}>
                ...
                <div className="sig-compare">
                  {[block.left, block.right].map((side, j) => {
                    ...
                    return (
                      <figure className="sig-compare__side" key={j}>
          ...
          case 'two-col': {
            ...
            return (
              <section className="sig-block" key={i}>
                {block.heading && <h2 className="sig-block__heading">{block.heading}</h2>}
                <div className={`sig-twocol${block.imageSide === 'left' ? ' is-reversed' : ''}`}>
                  <div className="sig-twocol__text sig-prose">
                    <SigMarkdown text={block.markdown} resolveImage={resolveImage} />
                  </div>
                  <figure className="sig-twocol__media">
          ...
          case 'embed':
            return (
              <section className="sig-block" key={i}>
                ...
                {block.caption && <p className="sig-embed__caption">{block.caption}</p>}
              </section>
            );
